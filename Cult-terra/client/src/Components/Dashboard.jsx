...
const DashBoard = (props) => {
  ...
  return (
    <div>
      <Navbar setIsOnDashboard={props.setIsOnDashboard} email={props.email} ></Navbar>

      <div className="px-4">
      <div
        className="flex justify-center items-center bg-white mx-auto my-20 max-w-7xl  rounded-lg p-10 "
      >
        ...
        <h1 className="text-3xl mb-9 text-center font-bold	 ">Book now</h1>
          <ul className="grid grid-cols-8 grid-rows-5 gap-8">
            {rooms.map((room,index)=> {
              ...
              <span className="absolute top-1 right-4 inline-flex items-center justify-center cursor-pointer hover:bg-yellow-500 px-2 py-1 text-xs font-bold leading-none text-red-100 transform translate-x-1/2 -translate-y-1/2 bg-yellow-600 rounded-full" onClick={ ()=>{setShowChangePriceModal(true); setShowRoomPrice(room.price);setRoomNo(room.roomNo)} }>{`Price- ${room.price}`}</span>
              <div
      ...
      {showbookedModal && <ModalBooked setOpenModal={setShowbookedModal} roomNo={roomNo} setRooms={setRooms}></ModalBooked> }
      {showChangePriceModal && <ChangePrice setOpenModal= {setShowChangePriceModal} showRoomPrice={showRoomPrice} setRooms={setRooms} roomNo={roomNo}></ChangePrice> }
    </div>
  );
}
